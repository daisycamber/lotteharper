...
#    frame.safe = not is_nude_fast(frame.still.path) # TODO FIX THIS
    if False and not frame.safe and settings.NUDITY_FILTER: # or not is_safe_image(frame.still.path):
        ...
        frame.save()
        return
    if frame.adjust_pitch and camera.name == 'private' and camera.user.profile.vendor:
        op_path = os.path.join(settings.MEDIA_ROOT, get_file_path(frame, 'frame.mp4'))
        from live.voice_changer import adjust_video_pitch
        new_path = adjust_video_pitch(path, op_path, camera.user.vendor_profile.pitch_adjust)
        try:
            os.remove(frame.frame.path)
        except: pass
        frame.frame = new_path
        frame.pitch_adjust = camera.user.vendor_profile.pitch_adjust
    if frame.animate_video and camera.name == 'private' and camera.user.profile.vendor:
        op_path = os.path.join(settings.MEDIA_ROOT, get_file_path(frame, 'frame.mp4'))
...
def process_recording(id):
    ...
    from live.models import VideoRecording, get_file_path, VideoCamera
    from django.utils import timezone
    from django.contrib.auth.models import User
    from django.conf import settings
    import os
