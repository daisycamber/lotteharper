...
def process_recording(id):
    ...
    if not recording.processing and not recording.processed and (not recording.last_frame or (recording.last_frame < timezone.now() - dt.timedelta(seconds=(settings.LIVE_INTERVAL/1000) * 6))): # 4 (the number is the gap, a larger number adds more length to the recording with a longer gap
